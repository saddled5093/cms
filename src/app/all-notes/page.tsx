...
import { FilePlus, ServerCrash, FilterX, Search, Calendar as CalendarIconLucide, XCircle, Loader2, Globe } from "lucide-react";
...
export default function AllNotesPage() {
  ...
  const [archiveFilter, setArchiveFilter] = useState<ArchiveFilterStatus>("unarchived"); // Default to unarchived for public view
  ...
  const fetchNotes = useCallback(async () => {
    // For public notes, no userId needed, API defaults to published.
    setIsLoadingNotes(true);
    try {
      const response = await fetch(`/api/notes?filter=public`);
      if (!response.ok) {
      ...
      console.error("Failed to load public notes from API", error);
      toast({
        title: "خطا",
        description: "بارگذاری یادداشت‌های عمومی از سرور ممکن نبود.",
        variant: "destructive",
      });
    } finally {
  ...
  useEffect(() => {
    // Public notes can be fetched even if no user is logged in,
    // but auth state is still useful for conditional UI elements if any.
    fetchNotes();
    fetchAvailableCategories();
  }, [fetchNotes, fetchAvailableCategories]);
  ...
  const clearFilters = () => {
    ...
    setArchiveFilter("unarchived"); // Default for public
    setTitleSearch("");
    setContentSearch("");
    setPhoneSearch("");
  ...
  // Delete, Archive, Publish actions are only for owners/admins, so they are disabled for public view if user is not owner.
  // The NoteCard component handles this logic based on currentUser.
  const handleDeleteNoteRequest = (noteId: string) => {
    if (!currentUser || (currentUser.role !== 'ADMIN' && notes.find(n => n.id === noteId)?.authorId !== currentUser.id)) {
  ...
  const handleToggleArchive = async (noteId: string) => {
    ...
    if (!noteToUpdate || !currentUser || (currentUser.role !== 'ADMIN' && noteToUpdate.authorId !== currentUser.id)) {
    ...
    try {
        // ... (rest of the logic remains the same as MyNotesPage)
        ...
        // If archived, remove from public list unless it's by current admin/author
        if (updatedNote.isArchived && !(currentUser && (currentUser.role === 'ADMIN' || currentUser.id === updatedNote.authorId))) {
  ...
  const handleTogglePublish = async (noteId: string) => {
    ...
    try {
        ...
        const updatedNoteRaw = await response.json();
        const updatedNote = processFetchedNote(updatedNoteRaw);
        // If unpublished, remove from public list unless current user is admin/author viewing it
        if (!updatedNote.isPublished && !(currentUser && (currentUser.role === 'ADMIN' || currentUser.id === updatedNote.authorId))) {
             setNotes(prevNotes => prevNotes.filter(note => note.id !== noteId));
        } else {
            setNotes(prevNotes => prevNotes.map(note => note.id === noteId ? updatedNote : note));
        }
  ...
  const filteredNotes = useMemo(() => {
    ...
    // Public page already fetches only published notes, so no client-side publish filter needed here.
  ...
  const activeFilterCount =
    ...
    (archiveFilter !== "unarchived" ? 1 : 0) + // Default unarchived is not an "active" filter
    (titleSearch ? 1 : 0) +
    (contentSearch ? 1 : 0) +
    (phoneSearch ? 1 : 0) +
    (dateRange?.from ? 1 : 0);
  ...
  if (isAuthLoading || (isLoadingNotes && !notes.length)) {
    return (
      <div className="container mx-auto p-4 md:p-8 text-center flex flex-col items-center justify-center min-h-[calc(100vh-8rem)]">
        ...
        <p className="text-lg text-foreground">در حال بارگذاری یادداشت‌های عمومی...</p>
      </div>
    );
  }
        ...
        <div className="mb-8 flex flex-col sm:flex-row gap-4 items-center">
          <div className="flex-grow">
            <h1 className="text-2xl font-headline text-primary flex items-center">
              ...
              همه یادداشت‌های منتشر شده
            </h1>
          </div>
          {currentUser && ( // Only show "New Note" if logged in
            <Button
              asChild
            ...
            <Card className="shadow-lg rounded-lg">
              ...
              <CardContent className="space-y-5">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(["unarchived", "archived", "all"] as ArchiveFilterStatus[]).map(status => (
                      <Badge
                        key={status}
                ...
                {/* Publish filter is not relevant for this page as it only shows published notes */}
              ...
              <div className="space-y-4">
                {filteredNotes.map((note) => (
                  <NoteCard
                    key={note.id}
                    note={note}
                    currentUser={currentUser as CurrentUser | null} // Can be null if user not logged in
              ...
              <div className="text-center py-12 text-muted-foreground h-full flex flex-col justify-center items-center">
                ...
                <p className="text-xl font-semibold mb-2 font-headline">
                  {activeFilterCount > 0
                    ...
                    : "هنوز یادداشت منتشر شده‌ای وجود ندارد"}
                </p>
                <p className="text-md">
                  {activeFilterCount > 0
